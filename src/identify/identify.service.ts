  ...
  async create(createIdentifyDto: CreateIdentifyDto) {
    ...
    if (
      ...
      for (let i = 0; i < findExistingData.length; i++) {
        ...
        /**
         * Checking the weather email or phoneNumber not equal to either
         * one of the existing data and linkPrecedence not equal to Secondary
        ...
        if (
          ...
          /** Checking the weather email or phoneNumber  equal to either
    ...
    const uniqueLinkedId = new Set(linkedIdFromData.filter(Boolean));

    //Removing duplicate id
    const uniqueId = new Set(idOfExistData.filter(Boolean));

    if (uniqueId != undefined || uniqueLinkedId != undefined) {
      //Filtering the data by id or linkedId from existing data
      const filter: Prisma.ContactWhereInput = {
        OR: [
          {
            id: {
              in: Array.from(uniqueId.size == 0 ? uniqueLinkedId : uniqueId),
            },
          },
          {
            linkedId: {
              in: Array.from(
                uniqueLinkedId.size == 0 ? uniqueId : uniqueLinkedId,
              ),
            },
          },
        ],
    ...
    for (let i = 0; i < filteredResponseData.length; i++) {
      ...
      if (structuringData.has(ele.linkedId?.toString())) {
        ...
        //Checking the phoneNumber is not null and checking the phoneNumber is existing in the map
        if (
        ...
        //Checking the linkPrecedence is not null and checking the linkPrecedence is existing in the map
